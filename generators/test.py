...
def run_test():
    ...
    # generate 100 graphs with 1024 nodes and 1024 edges
    ns = [1024]
    es = [1024]
    n_expts = 100

    # persist expected and actual degree counts to csv
    columns = ['experiment', 'k', 'expected', 'actual']
    df = pd.DataFrame(columns=columns)
    path = os.path.join('.', 'data', 'results', 'data_qual.csv')
    for expt_num in range(n_expts):
